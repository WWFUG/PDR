...
// isNxt[i]._bit denote if orderedNets[i] nxt state, 
// isNxt[i]._dontcare denotes if getLatch(i) need to be skipped
void V3SvrPDRSat::dfs(V3NetVec& orderedNets, bool isMonitor, vector<bool>& isNxt) {
  orderedNets.clear();
...
void V3SvrPDRSat::dfs_jump(V3NetVec& orderedNets, vector<bool>& isNxt, Cube* c) {
  orderedNets.clear();
  orderedNets.reserve(_ntk->getNetSize());
  _ntk->newMiscData();
  isNxt.resize(_ntk->getNetSize(), false);
  // pos.reserve(_ntk->getLatchSize());
  // pos.push_back(0);
  for (unsigned i = 0, n = _ntk->getLatchSize(); i < n; ++i) {
...
void V3SvrPDRSat::ternarySimulation(Cube* preImg, Cube* img, bool b, vector<bool>& input) {
  //TODO: SAT generalization
  // b=0 : in getBadCube() _monitor = 1
  // b=1 : in solveRelative
  //cout << "Ternary in " << (b ? "solveRelative()" : "getBadCube()") << " ... " << endl;
  vector <bool> isNxt;
  V3NetVec nets;
  if(b) dfs_jump(nets, isNxt, img);
  for(unsigned i=0; i<_L; ++i){
    _Value3List[_ntk->getLatch(i).id] = preImg->_latchValues[i];
    // if(b){
    //   if(img->_latchValues[i]._dontCare)
    //     _isNextS[ _ntk->getInputNetId(_ntk->getLatch(i), 0).id ] = false;
    // }
  }

  for(unsigned i=0; i<_I; ++i){
    _Value3List[_ntk->getInput(i).id] = _ntk->getInput(i).cp ? ~Value3(input[i], 0) :
                                                                Value3(input[i], 0);
  }
  for(unsigned i=0; i<_ntk->getConstSize(); ++i)
    _Value3List[_ntk->getConst(i).id] = (_ntk->getConst(i).cp) ? Value3(1, 0) : Value3(0, 0);

  bool flag = 0;
  for(unsigned i=0; i<_L; ++i){
    if(preImg->_latchValues[i]._dontCare) continue;

    V3NetId latch = _ntk->getLatch(i);
    _Value3List[latch.id] = Value3(); // dontcare 

    bool hold = true;
    if(b){   
      for(unsigned j=0; j<nets.size(); ++j){
        v3SimOneGate( nets[j] );
        if( isNxt[nets[j].id] ){ // is nxt state
          Value3 newV = _Value3List[nets[j].id];
          if( newV._dontCare ){
            hold = false; break;
          }
        } 
        // if( _isNextS[_allNets[j].id] ){ // is nxt state
        //   Value3 newV = _Value3List[_allNets[j].id];
        //   if( newV._dontCare ){
        //     hold = false; break;
        //   }
        // }    
      ...
      for(unsigned j=0; j<_monitorNets.size(); ++j){
        v3SimOneGate(_monitorNets[j]);
      }

      Value3 newV = _monitor.cp ? ~_Value3List[_monitor.id] : _Value3List[_monitor.id];
      if( newV._dontCare)
        hold = false;
    }

    if(hold){
      preImg->_latchValues[i] = Value3();
      flag = 1;
    }
    else{
      _Value3List[_ntk->getLatch(i).id] = preImg->_latchValues[i];
  ...
  //   for(unsigned i=0; i<_L; ++i){
  //     if(img->_latchValues[i]._dontCare)
  //       _isNextS[ _ntk->getInputNetId(_ntk->getLatch(i), 0).id ] = true;
  //   }
  // }
  //return preImg;
}
...
void V3SvrPDRSat::getSATAssignmentToCube(Cube* cube) {
  ...
  if( cube->_latchValues.empty() ) cube->initLatch();
  for (unsigned i = 0; i < _L; ++i) {
...
void V3SvrPDRSat::ternarySimInit(Cube* c, vector<bool>& ii) {
  // for latch
...
Cube* V3SvrPDRSat::getBadCube(unsigned depth) {
  ...
  if (assump_solve()) {
    ...
    ternarySimulation(c, NULL, 0, ii);
    c->setNext(NULL);
    c->setInput(ii);
    return c;
  } else {
    // c._latchValues.empty()
    return NULL;
  }
}
...
Cube* V3SvrPDRSat::UNSATGeneralizationWithUNSATCore(Cube* c, vector<Lit>& Lit_vec_origin, bool newCube) {
  vector<Lit> Lit_vec_new;
  vector<Value3> orig_latch;
  Lit_vec_new.resize(_L, Lit(0));
  if(!newCube) orig_latch.resize(_L, Value3());
  // cout << "UNSAT GEN w core " << c << endl;
  // cout << "new Cube ? " << newCube << endl;
  ...
  if(newCube){
    ...
    if (isInitial(tmpCube)) {
      assert(!c->_latchValues.empty());
      for (unsigned i = 0; i < _L; ++i){
        if( !c->_latchValues[i]._dontCare && c->_latchValues[i]._bit ){
          tmpCube->_latchValues[i] = c->_latchValues[i];
          break;
        }
      }
    ...
    //cout << "is C accessable ? " << endl;
    for(unsigned i=0; i<_L; ++i){
      orig_latch[i] = c->_latchValues[i];
      if (Lit_vec_new[i] == Lit(0))
    ...
    if(isInitial(c)){
      for(unsigned i=0; i<_L; ++i){
        if( !orig_latch[i]._dontCare && orig_latch[i]._bit) {
          c->_latchValues[i] = orig_latch[i];
          break;
        }
      }
    }
...
void V3SvrPDRSat::generalizeProof(Cube* c){
  // remove Lit from cube using sat solve
  // called by solveRelative()
  assumeRelease();
}
...
TCube V3SvrPDRSat::solveRelative(TCube s, size_t param) {
  ...
  if (assump_solve()) { //SAT
    ...
    if (param == 0) { // param == 0, return Cube_NULL,Frame_NULL
      ...
      if( param==2 ) return TCube(NULL, -1);

      vector<bool> ii;
      Cube* tmpCube = new Cube();
      ternarySimInit(tmpCube, ii);

      ternarySimulation(tmpCube, s._cube, 1, ii);
      tmpCube->setInput(ii);
      tmpCube->setNext(s._cube);
      TCube r(tmpCube, -1);
      return r;
    }
  } else { //UNSAT

    Cube* tmpCube = UNSATGeneralizationWithUNSATCore(s._cube, Lit_vec_origin, param!=0);
