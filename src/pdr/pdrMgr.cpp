  ...
  while (!Q.empty()) {
    ...
    if (!isBlocked(s)) {
      ...
      if (z._frame != -1) {
        ...
        if(debug) cout << "  Add Blocked cube in frame " << ((z._frame==INT_MAX) ? "INF" : to_string(z._frame)) << endl;
        addBlockedCube(z);
        ...
        z._frame = s._frame - 1;
        V3BitVecX assign;
        getInputAssign(assign);
        (z._cube)->setNext(s._cube, assign);
        _head = z._cube;
        Q.push(z);
